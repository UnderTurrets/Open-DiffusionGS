...
class GaussiansUpsampler(nn.Module):
    def __init__(self, config):
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(), 
            nn.Linear(config.width, 2 * config.width, bias=True)
        )
    ...
    def to_gs(self, gaussians):     # 把 dimenssion 分掉，分出对应的维度
        ...
        # 中位数-2.3，exp(-2.3)=0.1，初始倾向于产生 0.1 左右的scale，最大scale 0.3
        scaling = (scaling - 2.3).clamp(max=-1.20)
        # 中位数-2，sigmoid(-2)=0.119，初始倾向于产生 12% 左右的不透明度
        opacity = opacity - 2.0
...
class ImageTokenDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Linear(
                config.width,
                config.patch_size * config.patch_size * (3 + (config.gaussians_sh_degree + 1) ** 2 * 3 + 3 + 4 + 1),
                bias=False,
            )
        self.adaLN_modulation = nn.Sequential(
