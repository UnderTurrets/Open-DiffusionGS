...
class BaseDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        self.train_idxs = self.get_remaining_indices(self.cfg.gen_idxs, self.cfg.all_idxs)
    ...
    def get_remaining_indices(self, gen_idx: Optional[List[int]], all_idx: List[int]) -> List[int]:
        """
    ...
    def _get_data(self, index):
        ...
        train_idxs = self.get_remaining_indices(sel_gen_idxs,self.cfg.all_idxs)
        ...
        for image_file_dir in image_file_dirs:
            ...
            # https://kit.kiui.moe/camera
            # # Blender world -> OpenGL world: RotX(-90°) @ c2w
            c2w[1] *= -1 # 反转y轴
            c2w[[1, 2]] = c2w[[2, 1]] # 交换y轴和z轴
            # OpenCV cam -> OpenGL cam: c2w @ RotX(180°)
            c2w[:3, 1:3] *= -1 # 反转y轴和z轴
            ...
            depth = normald[...,3:]
            # 对于objaverse数据集，物体位于坐标原点 (0,0,0)，并且被缩放适配进一个 边长为 1 的单位立方体 (Unit Cube) 中
            # 0.867 = sqrt(3) * 0.5
            cond_cam_dis = np.linalg.norm(c2w[:3, 3:], 2)
            depth[depth < cond_cam_dis - 0.867] = 0
        ...
        # OpenGL to OpenCV camera for Gaussian renderer: c2w @ RotX(180°)
        ret['c2ws'][:, :3, 1:3] *= -1
        # World: converge to our training setting:（x+ right, y+ forward, z+ down）
        ret['c2ws'][:, [1, 2]] = ret['c2ws'][:, [2, 1]] 
        # breakpoint()
